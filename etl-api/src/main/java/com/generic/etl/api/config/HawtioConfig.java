...
/**
 * Hawtio configuration: static resources, auth mocks, Jolokia agent, and proxy.
 *
 * Architecture:
 *   Hawtio JS → /hawtio/jolokia/* (ProxyServlet) → /jolokia/* (AgentServlet) → MBeanServer
 *   Hawtio JS → /hawtio/* (HawtioServlet) for static resources and auth mocks
 *
...
public class HawtioConfig {
    ...
    /** Serves Hawtio static resources + auth mock endpoints at /hawtio/* */
    @Bean
    ...
    /** Jolokia 2.1.1 AgentServlet — raw JMX-to-HTTP bridge at /jolokia/* */
    @Bean
    ...
    /** Hawtio ProxyServlet — proxies /hawtio/jolokia/* → /jolokia/* */
    @Bean
    public ServletRegistrationBean<ProxyServlet> hawtioJolokiaProxyRegistration() {
        ServletRegistrationBean<ProxyServlet> reg =
            new ServletRegistrationBean<>(new ProxyServlet(), "/hawtio/jolokia/*");
        reg.setLoadOnStartup(3);
        reg.setName("hawtioJolokiaProxy");
        return reg;
    }
