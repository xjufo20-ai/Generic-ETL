...
public class EtlConfig {
    ...
    public AuditLog auditLog(ObjectMapper mapper) {
        return new AuditLog(Path.of("data"), mapper);
    }

    @Bean
    public LineageStore lineageStore(ObjectMapper mapper) {
        return new LineageStore(Path.of("data"), mapper);
    }

    @Bean
    public StateStore stateStore(ObjectMapper mapper, AuditLog auditLog) {
        return new StateStore(Path.of("data"), mapper, auditLog);
    }
    ...
    public PipelineExecutionService pipelineExecutionService(PipelineConfigParser configParser,
        ...
        return new PipelineExecutionService(configParser, transformPipeline, extractorRegistry,
                loadRouter, metrics, auditLog, lineageStore);
    }
